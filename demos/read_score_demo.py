...
VERBOSE = False  # set to True to output score data
...
sorted = myscore4.get_sorted_notes()
for note in sorted:  # myscore4 is a flat score
    if VERBOSE:
        print(
            f"  Note: pitch={note.pitch}, onset={note.onset:.3f},"
            f" duration={note.duration:.3f}"
        )
print(f"------- finished extracting {len(sorted)} notes -------")
