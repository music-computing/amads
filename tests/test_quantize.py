...
def test_quantize_basic():
    ...
    q_score = quantize(score, onsetres=1 / 32)
...
def test_quantize_separate_grids():
    ...
    q_score = quantize(score, onsetres=1 / 8, durres=1 / 16)
    notes = q_score.get_sorted_notes()
...
def test_quantize_filterdiv():
    ...
    # filterres = 1/32 implies threshold = 0.125
    # the second note has duration 0.1 < 0.125, so it should be removed
    q_score = quantize(score, onsetres=1 / 16, filterres=1 / 32)
    notes = q_score.get_sorted_notes()
...
def test_tied_deletes_front():
    ...
    q_score = quantize(score, onsetres=1 / 4, durres=1 / 4)
    notes = q_score.get_sorted_notes()
...
def test_tied_preserved():
    ...
    assert notes[0]._duration == 1.0
    tied_to = notes[0].tie
    assert tied_to is not None
    assert tied_to.onset == 4.0
    assert tied_to.duration == 1.0
    assert tied_to.tie is None  # no further ties
...
def test_tied_trims_end():
    ...
    tied_to = notes[0].tie
    assert tied_to is not None
    assert tied_to._duration == 1.0
    tied_to2 = tied_to.tie
    assert tied_to2 is not None
    assert tied_to2._duration == 0.5  # trimmed from 0.8 to fill to 2.5
    assert tied_to2.tie is None  # no further ties
...
def test_tied_deletes_end():
    ...
    note_b = Note(parent=part, onset=0.2, duration=0.2, pitch=60)
    note_c = Note(parent=part, onset=0.4, duration=0.5, pitch=60)
    note_a.tie = note_b
    note_b.tie = note_c
    ...
    assert notes[0]._duration == 0.5
    tied_to = notes[0].tie
    assert tied_to is not None
    assert tied_to.onset == 0.5
    assert tied_to._duration == 0.5
    assert tied_to.tie is None  # N3 was deleted
